...
func NewController(config *daemonconfig.Configuration,
	...
	if err := remoteVtepInformer.Informer().GetIndexer().AddIndexers(cache.Indexers{
		ByPodIPListIndexer: indexByPodIPList,
	}); err != nil {
	...
	ipSearch := func(ip net.IP, link netlink.Link) error {
		...
		if mac, exist := c.nodeIPCache.SearchIP(ip); exist {
			...
			if ipInstance != nil {
				...
				vtep, err := c.getRemoteVtepByPodAddress(ip)
				if err != nil {
...
func (c *Controller) iptablesSyncLoop() {
	iptablesSyncFunc := func() error {
		...
		if remoteOverlayExist {
			...
			for _, remoteSubnet := range remoteSubnetList {
				_, cidr, err := net.ParseCIDR(remoteSubnet.Spec.CIDR)
				if err != nil {
					return fmt.Errorf("parse remote subnet cidr %v failed: %v", remoteSubnet.Spec.CIDR, err)
				}

				c.getIPtablesManager(remoteSubnet.Spec.Version).
...
var indexByPodIPList cache.IndexFunc = func(obj interface{}) ([]string, error) {
	vtep, ok := obj.(*ramav1.RemoteVtep)
	if ok {
		podIPs := vtep.Status.PodIPList
		if len(podIPs) > 0 {
			return podIPs, nil
		}
	}
